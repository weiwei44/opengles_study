 ...
 * VBO是已经保存在服务端的顶点数据
 */
object VertexBufferHelper{
    fun readVertexBuffer(vertexDatas:FloatArray):Int {
        //向OpenGL服务端申请创建缓冲区
        ...
        // 把java数据放转至到native
        val vertexArray = VertexArrayHelper.readVertexBuffer(vertexDatas)
    ...
    fun readVertexBuffer(vertexDatas:MutableList<Float>):Int {
        //向OpenGL服务端申请创建缓冲区
        val buffers = IntArray(1)
        GLES20.glGenBuffers(buffers.size,buffers,0)
        if(buffers[0] == 0){
